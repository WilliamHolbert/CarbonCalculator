...
output: pdf_document
---
# Code/outline sourced from https://datascienceplus.com/time-series-analysis-using-arima-model-in-r/
...
df = datacenters %>%
  group_by(Year) %>%
  summarise(AvgPUE=mean(PUE),
            AvgSize=mean(Size..m.2.),
...
## CHANGE `df` to `dftemp` for filled in NA values
ts <- ts(df[,2], start=c(1989,1), end=c(2021,1),
                            frequency=2)
tstemp <- ts(dftemp[,2], start=c(1989,1), end=c(2021,1),
                            frequency=2)

plot(ts, xlab ="Yearly Data",
        ylab ="Avgerage yearly PUE",
        main ="Yearly Average PUE of Datacenters in US (no filled NA)",
        col.main ="darkgreen")

components.ts = decompose(ts)
plot(components.ts)

plot(tstemp, xlab ="Yearly Data",
        ylab ="Avgerage yearly PUE",
        main ="Yearly Average PUE of Datacenters in US (filled NA)",
        col.main ="darkgreen")

components.tstemp = decompose(tstemp)
plot(components.tstemp)
```
...
urkpssTest(tstemp, type=c("tau"), lags=c("short"), use.lag=NULL)
tsstationarytemp = diff(tstemp, differences=1)
plot(tsstationarytemp)
```
```{r}
acf(ts, lag.max=35)

acf(tstemp, lag.max=35)
```
```{r}
...
tsseasonaladjtemp = tstemp-components.tstemp$seasonal
tsstationarytemp <- diff(tsseasonaladjtemp, differences=1)
plot(tsstationarytemp)
```
```{r}
acf(tsstationary)
...
acf(tsstationarytemp)
pacf(tsstationarytemp)
```
...
fitARIMAtemp <- arima(tstemp, order=c(1,1,1), 
                  seasonal=list(order=c(1,0,0), period=2), 
                  method="ML")
coeftest(fitARIMAtemp)
```
...
acf(fitARIMAtemp$residuals)
boxresulttemp=LjungBoxTest (fitARIMAtemp$residuals,k=2,StartLag=1)
plot(boxresulttemp[,3],main= "Ljung-Box Q Test", ylab= "P-values", xlab= "Lag")
qqnorm(fitARIMAtemp$residuals)
qqline(fitARIMAtemp$residuals)
```
...
footemp=auto.arima(tstemp, trace=TRUE)
bartemp=predict(fitARIMAtemp, n.ahead=15)
```
